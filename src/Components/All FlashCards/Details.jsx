...
const FlashCardsDetails = () => {
  ...
  useEffect(() => {
    ...
    setTermImg(carddata.term[0].term_uploadimage ? (carddata.term[0].term_uploadimage) : ("Loading"))
  }, [carddata])

  // use useState hook to set default image of term_uploadimage if image is not uploaded by user
  const [TermImg, setTermImg] = useState("Loading");
  ...
  const setCard = (NewIndex) => {
    ...
    setTermImg(carddata.term[NewIndex].term_uploadimage ? (carddata.term[NewIndex].term_uploadimage) : ("Loading"))
  }
      ...
      <div className="pt-3 dark:text-white">
        ...
        <div className="flex flex-wrap space-between">
          ...
          <div className=" w-[250px] my-3  rounded-lg h-48">
            <div onClick={() => setVisible(true)} className="bg-white dark:bg-gray-800 flex cursor-pointer mb-4 drop-shadow-md hover:scale-110 rounded-lg w-[250px] p-2 h-10"style={{background:"orange"}}><TfiBackRight className='text-2xl mx-5'/>Share</div>
            ...
            <div onClick={() => { window.print() }} className="bg-white dark:bg-gray-800 flex cursor-pointer my-4 drop-shadow-md hover:scale-110 rounded-lg w-[250px] p-2 h-10"style={{background:"green"}}><BsPrinter className='text-2xl mx-5' />Print</div>
          </div>
